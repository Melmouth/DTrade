/* frontend/src/components/CompanyInfo.jsx */
import { useState, useEffect } from 'react';
import { Globe, DollarSign, TrendingUp, ShieldAlert, Cpu, ScanEye } from 'lucide-react';
...
export default function CompanyInfo({ ticker, isOpen, onClose, preloadedData }) {
  ...
  useEffect(() => {
    ...
    // 1. OPTIMISATION : Données préchargées (Snapshot)
    if (preloadedData) {
  ...
  return (
    <GlassModal
      isOpen={isOpen}
      onClose={onClose}
      title={ticker ? `${ticker} // INTELLIGENCE` : 'COMPANY DATA'}
      icon={ScanEye}
      maxWidth="max-w-4xl"
    ...
    >
      {loading ? (
        ...
        <div className="space-y-6">
            ...
            {/* FOOTER (Intégré au contenu car GlassModal gère le container) */}
